...
```{r}
### Load in data
sensor_data <- read_csv(file.choose())
```
...
process_sensors <- function(person_location) {
    ...
    temp_humidity_heat_index_day <<- temp %>%
      mutate(day = make_datetime(year = 2020, month = month, day = day)) %>%
      group_by(day) %>%
      summarise(mean_temp = round(mean(temperature),1),
                mean_humidity = round(mean(humidity),1),
                mean_heat_index = round(mean(heat_index),1)
    ...
    temp_humidity_heat_index_hour <<- temp %>%
      mutate(hour = make_datetime(year = 2020, month = month, day = day, hour = hour)) %>%
      group_by(hour) %>%
      summarise(mean_temp = round(mean(temperature),1),
                mean_humidity = round(mean(humidity),1),
                mean_heat_index = round(mean(heat_index),1)
    ...
    temp_humidity_heat_index_minute <<- temp %>%
      mutate(minute = make_datetime(year = 2020, month = month, day = day, hour = hour, min = minute)) %>%
      group_by(minute) %>%
      summarise(mean_temp = round(mean(temperature),1),
                mean_humidity = round(mean(humidity),1),
                mean_heat_index = round(mean(heat_index),1)
    ...
    excel_filename <- paste0(deparse(substitute(person_location)),"/",  deparse(substitute(person_location)), "_", date(now()), "temp_humidity_heat_index_means.xlsx")
...
process_sensor_data <- function(person_location) {
  ...
  temp <- person_location %>%  
    ...
    mutate(hour = hour(time)) %>%
    mutate(minute = minute(time)) %>%
    select(datetime, date, year, month, day, hour, minute, temperature, humidity) %>%
    mutate(heat_index = heat.index(t=temperature, rh=humidity, temperature.metric = "fahrenheit")) %>%
  ...
  temp_humidity_heat_index_day <- temp %>%
    ...
    summarise(mean_indoor_temperature = round(mean(temperature),1),
              ...
              mean_indoor_relative_humidity = round(mean(humidity),1)) %>%
